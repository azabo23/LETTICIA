...
dados_top20 <- dados_top20 %>%
  arrange(desc(NY.GDP.MKTP.CD)) %>%  # Agora ordem decrescente
  mutate(pais = factor(country, levels = country))  # Ordem no gráfico

# Gráfico de colunas VERTICAL
grafico_vertical <- ggplot(dados_top20, aes(x = pais, y = NY.GDP.MKTP.CD, fill = grupo)) +
  geom_col() +
  scale_fill_manual(
  ...
  labs(
    ...
    x = "País",
    y = "PIB (em dólares americanos correntes)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(face = "bold", size = 16, hjust = 0.5, family = "sans"),
    axis.title = element_text(size = 12, family = "sans"),
    axis.text.x = element_text(angle = 45, hjust = 1, size = 8),  # Diminuir o tamanho dos nomes no eixo X
